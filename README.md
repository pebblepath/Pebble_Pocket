...
| Audio | ES8311 codec, ES7210 echo cancellation, dual mics, 1 W speaker |
...
| 2 · 1 week | Learn and Calm from microSD. Upload in Portal, transcode, manifest sync on charge, selector and gallery faces |
...
- **Use ESP-IDF for audio, not Arduino.** The ES8311 and ES7210 share an I2S clock. Full duplex works when power rails come up first, then the I2S master clock, then both codecs as I2S slaves. Confirmed by a buyer on ESP-IDF v6.0.1; the Arduino examples hide the ordering.
- **Waveshare's Arduino sketches are unreliable.** Start from the MIT-licensed demo firmware on Waveshare's GitHub, not the wiki sketches.
- **Battery is the real budget.** Dim at 10 s, sleep at 30 s, wake on raise or press. Learn and Calm run at 40 percent brightness.
- **The speaker is 1 W.** Fine for one child at arm's length. A tabletop dock with a real speaker is a later option over the reserved I2C and UART pads.
